...
export function useSocial(
    ...
    const delayDebounceFn = setTimeout(async () => {
      try {
        const res = await fetch(
          `http://localhost:3000/api/users/search?q=${searchQuery}&current_user=${user}`,
        );
  ...
  useEffect(() => {
    if (!user) return;
    const endpoints = [
      { url: `pending/${user}`, setter: setPendingRequests, key: "requests" },
      { url: `sent/${user}`, setter: setSentRequests, key: "requests" },
